...
// クッキーの有効期限を延長
ini_set('session.cookie_maxlifetime', 60*60*24*30);
// セッションを使う
session_start();
// セッションIDを生成
...
// バリデーション関数(最大文字数チェック)
function validMaxLen($str, $key, $max = 6){
...
function makeRandKey($length = 8){
  static $chars = 'abcdefghijkmnopqrstuvwxyzABCDEFGHIJLKMNOPQRSTUVWXYZ0123456789';
  $str = '';
  for($i = 0; $i < $length; ++$i) {
...
function uploadImg($file, $key){
  ...
  if(isset($file['error']) && is_int($file['error'])) {
    try {
      // バリデーション
      // $file['error']の値を確認。配列内には「UPLOAD_ERR_OK」などの定数はphpでファイルアップロード時に自動的に定義される。定数には値として0や1などの数値が入っている。
      switch($file['error']){
        ...
        case UPLOAD_ERR_FORM_SIZE: // ファーム定義の最大サイズ超過した場合
          throw new RuntimeException('ファイルサイズが大きすぎます');
        default: // その他の場合
          throw new RuntimeException('その他のエラーが発生しました');
      }

      // $file['mine']の値は、MIMEタイプを自前でチェックする
      // exif_imagetype関数は「IMAGETYPE_GIF」「IMAGETYPE_JPEG」などの定数を返す
      $type = @exif_imagetype($file['tmp_name']);
      if(!in_array($type,[IMAGETYPE_GIF,IMAGETYPE_JPEG,IMAGETYPE_PNG],true)){
        // 第３引数には、trueを設定して厳密チェック
        throw new RuntimeException('画像形式が未対応です');
      }

      // ファイルデータからSHA-1ハッシュを取ってファイル名を決定し、ファイルに保存する
      // ハッシュ化しておきDBにパスを保存した場合にどの画像のパスなのか判断しやすくする
...
function pagination($currentPageNum, $totalPageNum, $link = '', $pageColNum = 5){
  ...
  for($i = $currentPageNum; $i <= $maxPageNum; $i++){
